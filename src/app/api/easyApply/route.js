    ...
    try {
      ...
      if (user.cv === "N/A") {
        return NextResponse.json(
          { message: "Please upload your CV before applying for a job" },
          { status: 400 }
        );
      }
      ...
      console.log(findApplication.length);
      ...
      if (job.skill_test) {
        ...
        const tests = await Tests.create({
          user_id: data.user_id,
          job_id: data.job_id,
          given: false,
        });
        ...
        const notEmployer = await Notification.create({
          employer_id: job.provider,
